...
整條鏈自動化。玩家決策集中在結構佈局：誰靠近誰、Refiner 位置能否同時受保護，又覆蓋到 Converter。
...
同一桶 Lumen，依 recipe 不同，產出不同 Dye——類比石油精煉。
...
燃燒路線玩家看到的，是「一座精煉廠」——高效、自動化、可擴展。
記憶路線玩家看到的，是「一套更精緻的靈魂消耗機器」。
機器越複雜，越不像在做壞事。Fragment 解鎖 Refiner / Converter，讓記憶路線玩家有能力看透自己在做什麼；純燃燒路線玩家只看到更高效的產線，直到結局 C 的揭示。

---

## 8.8 活體轉化路徑

在 **Reflection 空間**，走近 Garden 生物 + 長按轉化輸入（與補充燃料用**不同按鍵**，刻意區分）。

燃燒路線的終極形式：不是燃燒匿名掉落的 Lumen，而是在**餘燼**面前，親手處理一個你認識的存在。
...
**設計諷刺：你記得越多，燒掉牠越划算。** 記憶本身成了可以變現的資產——記憶路線玩家的最大誘惑，也是這個選擇有重量的原因。

### **餘燼**反應

**餘燼**在 Reflection 空間，看著這件事發生。不評判，不阻止。

若轉化的是**已命名且 Codex 完整的生物**，下次 run 開始前 **餘燼**說一句話——每個物種不同，永遠是陳述，非評判。設計原則同 §11 死亡台詞：從不以明顯方式給予安慰。
...
**記憶深度加成在衰減前作用：** §8.8 的加成（×1.5 + Ash Dye、最深層 → 高純度 + Remnant 配方）作用於 `base_yield`，然後才乘以 decay 係數。「第一個燒掉深度記憶生物」因此是最大化收益的時機——高風險的單一選擇維持其重量，大量轉化自我限制。

相關常數見 §9.2（`conversion_decay`、`dark_period_floor` 等）。

### 待解問題

- **餘燼**活體轉化台詞：每物種各一句（共 5 句）——獨立敘事寫作任務
...
階段 I — 儲量流失（Draining）
  觸發：霧接觸結構
  效果：capacity 緩慢下降，fuel 被優先抽取（儲存的 Lumen 流失）
  視覺：結構邊緣出現微弱黑色紋路
  自動修復仍運作
...
激活門檻（`capacity × activation_pct`）動態縮放——capacity 蝕至接近零時，結構進入永久死狀態。
...
設計意圖：迫使玩家在波次中持續介入，而非把結構放著讓它們自己跑。
