...
Cendres 不是射擊遊戲。玩家沒有力量優勢。Lantern 是工具，不是安慰用的武器——主動使用它會消耗燃料。結構撐起主要防線；玩家延伸、修補。
...
氣質決定每隻 Void 在場上的個體行為模式。它是讀懂戰況的基礎——同一物種，應對方式可能截然不同。
...
**Curious**
向最近光源漂移；僅在緊鄰或依靠 `light_tolerance` 抵抗靈光傷害時攻擊。**Dim → 靠近但停止攻擊**（現有規則）；Void-Tempered Lantern 可使其接近馴服。最友善的傾向，捕捉也最容易。

**Timid**
貼著暗區移動，繞開光源半徑；只要有暗路可走，絕不穿越明亮區域。**Ember Tether Line 會驅離牠們，迫使繞道**（現有規則，見 §8.13）。從側翼和缺口攻擊；受威脅時（附近 Flare / HP 低）向黑暗逃竄。
...
> 可選：Feral 狀態下死亡掉落略多 Lumen（抵消中斷捕捉的風險代價）。
...
| Territorial | 忽略（目標鎖定）| 受傷，持續推進 | 若有路則繞道 | Shield Emitter 護住目標；Vigil_Lamp 扼喉 / Flare 攔截 |
| Feral | 忽略 | 受傷，衝刺 | 忽略 | Flare 爆發擊倒或牆壁擋住；等計時器 |

> **Dim 是真正的雙刃——**：安撫 Curious 的同時，在暗中推進的 Timid 反而更危險。分離傾向設計的核心回報在此。
...
**啟動時間作為自然摩擦：** 結構放置或移動後，需 Lumen 灌注才激活。這段暗期是波次中調整的代價——提前在 Prep 階段佈置的自然動機，由此成立，而非強制規則。

**暗期規則（Activation Dark-Period）：**
- 觸發時機：**僅放置或移動**觸發——單純補燃料不觸發暗期
- 暗期內：無攻擊、無被動效果、無燃料消耗，但仍**脆弱**（Void 接觸仍造成耐久傷害）
- 持續時長：`dark_period_floor + capacity × dark_period_scale`（起始值：floor ≈ 1.0 秒；scale 讓大型結構落在 3–5 秒）
  - 小型 Beacon_Pillar 快速重激活；大型重要結構是波次中移動的真實代價
- Lumen 灌注的既有啟動門檻（`activation_pct`）保留；暗期計時器是**額外的時間門檻**，兩者並行
...
| 結構 | 覆蓋形狀 | 戰術用途 | 偏好 Dye |
|---|---|---|---|
| **Beacon_Pillar**（燈柱） | 360° 中距離 | 通用覆蓋，佈陣骨幹 | Weave（耐久）、Ember（持燃） |
| **Vigil_Lamp**（守夜燈） | 窄錐形長射程 | 走廊控制、跨距離橋接 | Frost（射程延長）、冥（偵測） |
| **Flashpoint**（閃點） | 短距離觸發爆炸 | 接觸時爆發，被動等待 | Ember（效率 +40%） |
| **Void_Mirror**（虛鏡） | 近感觸發脈衝 | 需馴化 Void 燃料；冥 Dye 觸發範圍加倍 | 冥 |
...
本節僅適用 §8.4 拉取捕捉（HP 門檻觸發的 Tether）。**§8.5 Tether 聆聽模式為不同動作（無 HP 門檻、無拉取），不適用以下規則。Behemoth 無法用 Tether 捕捉，本節對其完全不適用（見 `05-run-structure.md` §7.5）。**
...
在玩家通過 **餘燼**對話或 Codex 深度解鎖、充分理解「Lumen = 靈魂」後，Tether 出現第二種用法：
...
**揭露前：** 這個行為存在，但沒有標示。玩家若偶然長按 Tether 而未拉取，Void 的異常靜止只像是動畫 bug。揭露後，他們會意識到：那一直都是一個選項。
