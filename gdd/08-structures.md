...
充能炮臺是第一種**顯式納入**激活門檻機制的戰鬥結構：不達門檻就無法充能；充完沒打出，還會損耗。
...
**核心張力：** 炮臺在有目標時立即發射（charge 達 fire_threshold 即觸發）；如果 Void 不經過射程，charge 累積到 max 後開始損耗——放錯位置的炮臺不只是廢的——還主動消耗 Lumen。
...
偵測的依據是**異常的暗**，不是主動照亮——Void 所在的暗區，比預期背景暗更深、無反射。Vigilance Lens 因此對任何物種都有效，包括 Lurker。

### Client Device（隨身裝置）

裝置由玩家隨身攜帶，顯示在畫面角落。可以「抬起」切換至完整視圖：
...
切換頻率需要停止移動約 1 秒（調整需要穩定）。在波次中換頻有注意力代價——這個選擇，本身是資訊決策的一部分。

### Lurker 的特殊簽名

Lurker 沿光覆蓋邊緣移動，在雷達上呈現**弧線滑動的深色塊**，而非直線衝入的點。有經驗的玩家能從移動軌跡的形狀辨認物種，不需要直接看到牠。
...
Echo Marker 說的是**過去**，非預言。Composite 波次的組合可能與前幾波完全不同。過度依賴熱區資料的玩家會被突然的路線改變殺個措手不及。

這個不確定性是刻意保留的——是資訊工具，不是解答。
...
連接兩個結構節點的供給線，傳輸 Lumen 與 Dyed Lumen。自身無戰鬥能力；遭 Void 攻擊會斷線。
...
Tether Line 因此不只是後勤工具，也直接影響戰術覆蓋範圍——一條供給線，能讓遠端的孤立 Vigil_Lamp 覆蓋到原本碰不到的走廊入口。
...
Weave 生成者（Siege 波次）是最大威脅：牠製造暗區、切光覆蓋，現在也可切供給線，讓 Siege 波次同時威脅防線和後勤。
...
**設計意圖：** 靜默修復站提供階段 I 的長期防護，但在霧侵蝕最嚴重的時候——也就是最需要它的時候——自動失效。是預防工具，不是救援工具。這個設計，讓玩家無法純靠修復站就無視霧侵蝕。
...
Gnasher 快速衝擊，Shield Emitter 的短暫保護場在此情境最有效——保護場撐住最初的衝擊，讓玩家有時間趕到補充燃料或重新佈局。
...
**代價：** Emitter 的 fuel 在大量觸發時消耗得很快；Volume 波次後期，可能因自身 fuel 耗盡而關機。保護空窗出現的時間點，可能剛好是最糟的——這是刻意設計的風險。
