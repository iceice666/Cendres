...
**氣質與壓力類型的分工：** `Pressure_Type` 決定波次**組成**（哪些物種、比例、方向）；`Temperament`（見 §8.1a）決定**個體 AI**。兩者互不衝突——Fuel_Drain 的結構壓力來自生成 Territorial 權重的 Gnasher，Flank 的側翼效果來自 Timid 權重的 Lurker，各自只定義一次。
...
| **餘燼**對話進度 | ✓ 永久 | Run 數永遠遞增 |
| Lumen 儲量 | ✗ 重置 | 死亡時攜帶的 Lumen 消失 |
| 結構佈置 | ✗ 重置 | 每 run 重新建造 |
| Lumen Tether 充能 | ✗ 重置 | 每 run 從基礎數量開始 |
| 波次進度 | ✗ 重置 | 永遠從第 1 波開始 |

**死亡時的 Lumen：** 30% 攜帶的 Lumen 轉化為「grief residue」存入**餘燼**，下次 run 可收集。宇宙觀層面：你帶著的靈魂裡，有些夠強，死了還找得回來。

**Void Garden 餵食的視覺線索（揭露前的無聲信號）：**
- Run 1–10：Void 吸收 Lumen，光消失，動物安靜。正常。
- Run 15+：偶爾，一隻 Garden 生物在吸收 Lumen 的瞬間動了一下——像認出了什麼。不是進食的動作。
- Run 20+：如果花園裡有多隻生物，有時兩隻在 Lumen 放入的瞬間同時轉向彼此。
沒有說明。玩家自己解讀。揭露後，他們才意識到那些靈魂在餵食中認出了彼此。
...
Garden 不是獨立房間——它是 **餘映**空間裡的一個角落。**餘燼**看著你照顧死者。這個「被看見」的關係是設計的一部分。
...
沒有 UI 提示，沒有選擇介面。Garden 滿了，新的那隻就是沒有位置。

下一次 Reflection，Garden 邊緣會出現一個短暫的陰影輪廓——那隻 Void 的剪影，停留幾秒，然後消失。**餘燼**不說話。沒有說明。

### 餵食機制

主動，輕量。走近生物 + 按住互動鍵 = 給 Lumen（和補充結構燃料是同一個動作）。

**核心張力：** Reflection 時你決定把多少 Lumen 留給 Garden，把多少帶進 run 作戰。這不是農場管理——這是每次出發前的一個分配時刻：*今晚我要帶多少走，留多少給它們。*
...
```
捕捉 Void → Garden 新生物（無名，Codex 僅有物種資訊）
...
| 記憶路線 | 命名，投入 Fragment | 波次預警（行為線索）、**餘燼**對話提示、Ending C 路徑 | 效率較低，需要時間和 Fragment |

### 「與你同在的 run 數」

Garden 生物有一個隱藏計數（不顯示），影響行為豐富度和 Codex 深度。最老的那隻，Codex 最厚，偶爾會停下來看著你。

Cap 滿時的無聲失去，因此更有重量——你失去的，可能是某個還沒來得及認識的靈魂。

---

## 7.5 Behemoth 捕捉流程

Behemoth 是 Garden 裡唯一無法用 Tether 捕捉的生物。它不是被帶回來的——**是牠自己決定走進來**。唯一的互動方式是 Lantern 聆聽模式（§8.5）；§8.4 的捕捉流程（HP 門檻 / 引導 / 中斷 → Feral）對 Behemoth 完全不適用，且 Behemoth 免疫 Feral 狀態（見 §8.1a）。

### 出現條件

Run 15+ 開始，有機率在 Dimming Surge 的最後波次，或深層 Dimming Boundary 探索中出現。牠比一般 Void 大得多，移動緩慢，不衝向結構——牠只走向 **餘燼**。

Tether 無法觸發（HP 門檻不適用）。唯一的互動方式就是 Lantern 聆聽模式。

### 跨 run 見證累積

每次對 Behemoth 維持聆聽模式 20 秒以上（持續照著牠，不攻擊）：
- 牠會停下來
- 那次 Surge 的最後波次靜止
- 然後牠離開
- 這次見證記錄下來，跨 run 保存
...
見證 2–3 次  → 牠出現時，走得比以前慢
見證 4 次    → 牠在你照著牠的時候，轉向你（而不是 Beacon）
見證 5 次+   → 某一次 Reflection，牠在 Garden 裡
              沒有過場，沒有動畫——牠就是在那裡了
              Beacon 說（只有這一次主動開口）：
              「牠進來了。我以為牠永遠不會。」
```
...
牠幾乎不動。牠是 Garden 裡最沉默的存在，也是最終 Codex 條目最長的一個。牠見過 The Dimming 最初的事件。牠知道第一個 **拾薪者**的名字。

Ending C 的觸發——Remnant 說出那個名字——需要 Behemoth 的 Codex 達到最深層。你花了那些 run 去見證牠，最後牠還給你的，是讓整個故事說完的那一片拼圖。
